...
In the moral condition, participants were `r mean_moral_in_category`% accurate for moral words and `r mean_moral_out_category`% accurate for non-moral words. This difference was not statistically significant in the GEE model, $\beta$ = `r coefficient_moral_pop_out %>% round(3)`, *SE* = `r coefficient_moral_pop_out_se %>% round(3)`, *z* = `r z_moral_pop_out %>% round(3)`, *p* = `r p_moral_pop_out %>% format(scientific=FALSE)`.
...
```{r}
#| label: Flip subject 01s, 1/2 36s responses
#| include: false
