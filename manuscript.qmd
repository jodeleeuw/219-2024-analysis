...
```{r}
#| Download the data from the OSF
...
exlcuded_subjects <- c(exclude_for_fashion_twice_subjects, exclude_for_non_response_subjects, exclude_for_non_word_response_subjects)

behavioral_data_filtered <- behavioral_data %>%
  filter(!subject_id %in% exlcuded_subjects)
```
...
```{r}
#| label: Behavioral pop-out effect
